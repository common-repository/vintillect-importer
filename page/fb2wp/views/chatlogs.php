      ...
      <div id="chatlogs-view">
        <?php
          if ( (strpos($configSettings['options'], 'chatlogs') !== false) && (strpos($configSettings['jsonAvailable'], 'chatlog.zip') !== false) ) {
            ...
            ?>
              <div class="alert alert-warning purchase-warning" id="purchase-warning" role="alert">Chat history available for purchase.<br/>Check the <a href="<?php echo esc_url('https://vintillect.com/vintillect-importer/fb2wp/cart.php?uploadid=' . $uploadId); ?>" target="_blank">the Messenger Chat History option</a>.</div>
            <?php
          }
        ?>
