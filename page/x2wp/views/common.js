const viCommonObj = {
  ...
  formatPostContent: function(post) {
    ...
    if ('reply' in post && post['reply']) {
      const replyUrl = 'https://twitter.com/' + post['reply']['screenName'] + '/status/' + post['reply']['tweetId'];
      content =  `<!-- wp:paragraph -->\n<p>Replied to: <a href="${replyUrl}" target="twitter">${replyUrl}</a></p><!-- /wp:paragraph -->\n\n` + content;
    }
    ...
    if (media.length > 0) {
      ...
      if (! window['vi_posts_var']['availableOptions'].includes('media')) {
        ...
        content = `\n<div style="position:relative;"><button popovertarget="${dateGmtId}" popovertargetaction="show" class="alert alert-warning float-end">Cannot post</button>\n<div id="${dateGmtId}" popover class="alert alert-warning"><p>Media cannot be imported without<br/><a href="https://vintillect.com/vintillect-importer/x2wp/cart.php?uploadid=${viCommonObj.uploadId}" target="_blank">the Photo &amp; Video Albums option</a>.</p></div></div>` + content;
      }
    }
