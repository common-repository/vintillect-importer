...
class Vintillect_X2wp_Ajax {
  ...
  public function init() {
    ...
    $this->ajaxHelper = new Vintillect_Ajax_Helpers('x2wp', $this->mediaUrlBase, false, $this->configSettings);

    add_action('wp_ajax_x2wp_post_ajax_action', array($this, 'postAjaxCallback'));
    add_action('wp_ajax_vintillect_media_ajax_action', array($this->ajaxHelper, 'mediaAjaxCallback'));
    add_action('wp_ajax_x2wp_update_config', array($this, 'updateConfig'));

    $this->availableOptions = isset($this->configSettings['options']) ? explode(',', $this->configSettings['options']) : [];
    $this->allowMedia = in_array('media', $this->availableOptions);
  ...
  private function formatMultiplePosts($posts, $postId, $siteTzDate, $doAddTags, $isSubTweet) {
    if (!$posts || empty($posts)) { return []; }
    $hasGroupedPosts = (count($posts) > 1) || (array_key_exists('subTweets', $posts[0]));
    $formattedPosts = [];
    ...
    foreach ($posts as $post) {
      ...
      if (array_key_exists('subTweets', $post)) {
        $formattedContent .= implode("\n\n", $this->formatMultiplePosts($post['subTweets'], $postId, $siteTzDate, $doAddTags, true) );
      }
      if (array_key_exists('repliedToTweetUrl', $post)) {
        $formattedContent = "<!-- wp:paragraph -->\n<p>Replied to: <a href=\"{$post['repliedToTweetUrl']}\" target=\"twitter\">{$post['repliedToTweetUrl']}</a></p><!-- /wp:paragraph -->\n\n" . $formattedContent;
      }
